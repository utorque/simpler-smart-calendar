...
- Toolbar = three custom buttons only: `add-task` (promote selection to task), `cleanify` (tidy whole note via LLM), `undo-cleanify` (single-step restore). The same actions are also surfaced as standalone HTML buttons (`#notePromoteBtn`, `#cleanifyBtn`, `#undoCleanifyBtn`) below the editor.
